...
@onready var bouquet_scene = preload("res://src/flowers/bouquet.tscn")
var base_speed = 200
var moving_time = 0
...
func move_to_point(new_target_point):
	_unset_interaction_target()
...
func _drop_item(item):
	var current_position = item.global_position
	item.reparent(get_parent())
	item.global_position = current_position
	if item is WateringCan:
		item.sprite.rotation_degrees = 0
...
func open_bag_crate_ui(ui: BagCrateUI):
	if held_item == null or held_item is SeedBag:
		ui.open()
...
func sell_bouquet(customer: Customer):
	if held_item is Bouquet:
		var bouquet = remove_held_item()
		customer.give_bouquet(bouquet)
...
func _flip_h(val: bool):
	body_sprite.flip_h = val
	front_wing_sprite.flip_h = val
	back_wing_sprite.flip_h = val
	pollen_sprite.flip_h = val
...
func _set_flippables(positive: bool):
	for flipabble in flippables.get_children():
		flipabble.position.x = abs(flipabble.position.x)
		if not positive:
